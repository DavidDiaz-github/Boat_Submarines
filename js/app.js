const boatApp = {
    ...
    reset() {
        this.boat = new Boat(this.ctx, this.canvasSize.w / 2 - 200, this.canvasSize.h / 2 - 350, 200, 100, this.canvasSize, 'boat-sprite.png')
   ...
   clearScreen() {
    ...
    winGame() {
        if (this.submarineBoss.lives === 0 && this.submarines.length === 0) {
            ...
            const button = document.querySelector('.newGame2')
            button.onclick = () => {
                clases.toggle("display")
                this.score = 0;
                boatApp.init('canvas')
